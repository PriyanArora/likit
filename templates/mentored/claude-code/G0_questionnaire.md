...
2. **guide.md** — developer profile, scopes, TDD targets, red lines from manifest
3. **BuildFlow.md** — all phases with name, goal, checkboxes, proof, commit
4. **Progress.md** — phase names and checkboxes, category noted

Granularity rule for every phase: each checkbox is the single smallest action completable in one sitting, independently verifiable by a specific command, test output, or demonstration, and depends on at most one prior unverified step. Split any checkbox that needs "and" to describe it. There is no maximum number of steps per phase — more granular is better. Every phase keeps its Proof line.
...
**Step 10 — Store `guide.md` in memory.**

**G0 pass condition:** Manifest complete. ProjectSummary.md populated. Unused templates deleted. Zero placeholders in any Likit file. User confirmed manifest before generation.
